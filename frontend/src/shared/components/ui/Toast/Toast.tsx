...
export const ToastProvider: React.FC<ToastProviderProps> = ({
  ...
  const error = useCallback((message: string, title?: string) => {
    return addToast({ message, title, variant: 'error', duration: 7000 });
  }, [addToast]);

  const warning = useCallback((message: string, title?: string) => {
    return addToast({ message, title, variant: 'warning', duration: 6000 });
  }, [addToast]);
    ...
    >
      <div className="flex items-start gap-3">
        ...
        <div className="flex-shrink-0 mt-0.5">
          <svg className={cn('w-5 h-5', styles.icon)} fill="currentColor" viewBox="0 0 20 20">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className={cn('text-sm', styles.text)}>{toast.message}</p>
          {toast.action && (
            <button
              onClick={() => {
        ...
        <button
          ...
          className={cn(
            ...
            toast.variant === 'info' && 'focus:ring-blue-500'
          )}
          aria-label="Close"
        >
          <svg className={cn('w-5 h-5', styles.icon)} fill="currentColor" viewBox="0 0 20 20">
